...
Basel IRB Risk-Weighted Assets total \$6,033,111,741 at 161.8\% RWA density.
IFRS 9 Expected Credit Loss provisions total \$1,182,715,504 (coverage: 31.710\%).
\end{abstract}
...
\midrule
Expected Loss (EL) & \$279,666,552 & Lifetime Expected Loss projection of current portfolio. \\
Portfolio EL Rate & 7.50\% & Underwriting expected loss density. \\
\midrule
Basel IRB Total RWA & \$6,033,111,741 & Risk-Weighted Assets calculated using retail ASRF formula. \\
...
\midrule
Portfolio IFRS 9 ECL & \$1,182,715,504 & Probability-weighted Stage 1, 2, \& 3 Expected Credit Loss. \\
Portfolio ECL Coverage & 31.710\% & Capital coverage buffer (Total ECL / Total EAD). \\
\bottomrule
\end{tabular}
\end{table}
...
The scorecard demonstrates stable, high-contrast risk separation across distinct macroeconomic cycles. Under Basel capital standards, the risk-sensitive Internal Ratings-Based (IRB) approach identifies a capital requirement of \textbf{\$482,648,939} compared to \textbf{\$223,786,919} under the Standardised Approach. This capital surcharge of \textbf{\$258,862,020} reflects the elevated risk profile (PD/LGD) of the LendingClub consumer portfolio, demonstrating that a flat 75\% risk weight under the Standardised Approach materially undercapitalises this retail asset class.
...
\begin{sloppypar}\noindent
The final set is: \texttt{grade\_enc},\allowbreak \texttt{term\_enc},\allowbreak \texttt{loan\_to\_income},\allowbreak \texttt{fico\_range\_high},\allowbreak \texttt{revol\_util\_x\_new\_acc},\allowbreak \texttt{new\_accounts\_ratio},\allowbreak \texttt{dti},\allowbreak \texttt{bc\_open\_to\_buy},\allowbreak \texttt{num\_tl\_op\_past\_12m},\allowbreak \texttt{annual\_inc},\allowbreak \texttt{avg\_cur\_bal},\allowbreak \texttt{mo\_sin\_rcnt\_tl},\allowbreak \texttt{mths\_since\_recent\_inq},\allowbreak \texttt{inq\_last\_6mths\_per\_month},\allowbreak \texttt{num\_rev\_tl\_bal\_gt\_0},\allowbreak \texttt{mths\_since\_recent\_bc},\allowbreak \texttt{mort\_acc}.
\end{sloppypar}
...
\texttt{annual\_inc} & 0.0365 & Weak & \textbf{Retained} \\
\bottomrule
...
\texttt{grade\_enc} & -0.5154 & 0.0106 & -48.617 & 0.0000$^{***}$ \\
\texttt{term\_enc} & -0.6523 & 0.0125 & -52.003 & 0.0000$^{***}$ \\
\texttt{loan\_to\_income} & -0.4486 & 0.0158 & -28.397 & 0.0000$^{***}$ \\
\texttt{fico\_range\_high} & -0.2673 & 0.0190 & -14.085 & 0.0000$^{***}$ \\
\texttt{revol\_util\_x\_new\_acc} & -0.0490 & 0.0240 & -2.039 & 0.0414$^{**}$ \\
\texttt{new\_accounts\_ratio} & -0.4533 & 0.0261 & -17.398 & 0.0000$^{***}$ \\
\texttt{dti} & -0.3299 & 0.0221 & -14.909 & 0.0000$^{***}$ \\
\texttt{bc\_open\_to\_buy} & -0.3961 & 0.0272 & -14.575 & 0.0000$^{***}$ \\
\texttt{num\_tl\_op\_past\_12m} & -0.2285 & 0.0355 & -6.429 & 0.0000$^{***}$ \\
\texttt{annual\_inc} & -0.4587 & 0.0325 & -14.101 & 0.0000$^{***}$ \\
\texttt{avg\_cur\_bal} & -0.3561 & 0.0334 & -10.669 & 0.0000$^{***}$ \\
\texttt{mo\_sin\_rcnt\_tl} & 0.0183 & 0.0396 & 0.462 & 0.6441 \\
\texttt{mths\_since\_recent\_inq} & -0.0281 & 0.0518 & -0.543 & 0.5869 \\
\texttt{inq\_last\_6mths\_per\_month} & -0.5141 & 0.0400 & -12.845 & 0.0000$^{***}$ \\
\texttt{num\_rev\_tl\_bal\_gt\_0} & -0.3302 & 0.0347 & -9.519 & 0.0000$^{***}$ \\
\texttt{mths\_since\_recent\_bc} & -0.3771 & 0.0378 & -9.989 & 0.0000$^{***}$ \\
\texttt{mort\_acc} & -0.3898 & 0.0424 & -9.197 & 0.0000$^{***}$ \\
\midrule
...
\midrule
\texttt{grade\_enc} & (-inf, 1.50) & 1.2418 & -0.5154 & 59,675 & 5.62\% & 44.4 \\
\texttt{} & [1.50, 2.50) & 0.4866 & -0.5154 & 107,931 & 11.25\% & 33.2 \\
\texttt{} & [2.50, 3.50) & -0.1063 & -0.5154 & 96,905 & 18.65\% & 24.3 \\
\texttt{} & [3.50, 4.50) & -0.4971 & -0.5154 & 58,848 & 25.31\% & 18.5 \\
\texttt{} & [4.50, inf) & -0.9212 & -0.5154 & 39,958 & 34.12\% & 12.2 \\
\texttt{} & Special & 0.0000 & -0.5154 & 0 & 0.00\% & 25.9 \\
\texttt{} & Missing & 0.0000 & -0.5154 & 0 & 0.00\% & 25.9 \\
\texttt{term\_enc} & (-inf, 48.00) & 0.3076 & -0.6523 & 270,363 & 13.16\% & 31.7 \\
\texttt{} & [48.00, inf) & -0.6604 & -0.6523 & 92,954 & 28.52\% & 13.5 \\
\texttt{} & Special & 0.0000 & -0.6523 & 0 & 0.00\% & 25.9 \\
\texttt{} & Missing & 0.0000 & -0.6523 & 0 & 0.00\% & 25.9 \\
\bottomrule
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        \textbf{Expected Loss (EL)} & \$279,666,552 & \$820,418,154 & 193.4\% \\
        \textbf{Risk-Weighted Assets (RWA)} & \$6,033,111,741 & \$8,602,848,074 & 42.6\% \\
        \textbf{Capital Requirement (8\%)} & \$482,648,939 & \$688,227,846 & 42.6\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
\midrule
Expected Loss (EL) & 279.79 \\
Value-at-Risk (VaR 99.9\%) & 716.80 \\
Expected Shortfall (ES 99.9\%) & 776.89 \\
Unexpected Loss (UL $=$ VaR $-$ EL) & 437.01 \\
\midrule
...
Model validation is performed on the completely held-out Out-of-Time (OOT) dataset ($2016$--$2018$). The PD scorecard shows stable performance with an OOT AUC of \textbf{0.7004} (bootstrap 95\% CI [0.6990, 0.7018], 500 resamples) and a Kolmogorov-Smirnov (KS) statistic of \textbf{0.2894}.
...
Calibration Slope & $\approx 1.00$ & 0.9458 & 0.8845 & -0.0613 $\times$ \\
Calibration Intercept & $\approx 0.00$ & 0.5558 & 0.0060 & -0.5498 \checkmark \\
Expected Default Rate & = Actual & 16.61\% & 24.22\% & +7.61\% \checkmark \\
Actual Default Rate & 26.33\% & 26.33\% & 26.33\% & +0.00\% \\
Hosmer-Lemeshow $p$ & $>0.05$ & 0.0000 & 0.0003 & $\ast$ below 0.05 \\
\bottomrule
...
2008 & 2,393 & 13.64\% & 20.73\% [19.2\%, 22.4\%] & 0.66 {\checkmark} \\
2009 & 5,281 & 11.52\% & 13.69\% [12.8\%, 14.6\%] & 0.84 {\checkmark} \\
2010 & 12,537 & 14.06\% & 14.01\% [13.4\%, 14.6\%] & 1.00 {\checkmark} \\
2011 & 21,721 & 14.62\% & 15.18\% [14.7\%, 15.7\%] & 0.96 {\checkmark} \\
...
\begin{table}[H]
\centering
\caption{ECL What-If Sensitivity (Baseline ECL $=$ \$1,182.7M; note that this Baseline-only ECL reference differs from the probability-weighted total ECL in Table~\ref{tab:exec_summary} as it excludes the Upside/Downside macro scenario shocks mandated by IFRS~9)}
...
\midrule
PD +20\% & 1,210.83 & +28.11 & +2.4\% \\
PD +50\% & 1,252.75 & +70.03 & +5.9\% \\
LGD +10pp & 1,314.21 & +131.49 & +11.1\% \\
EAD +15\% & 1,360.12 & +177.41 & +15.0\% \\
Combined & 1,423.12 & +240.40 & +20.3\% \\
COVID-like (PD x2.5) & 1,547.74 & +365.03 & +30.9\% \\
GFC-like (PD x3.0) & 1,602.72 & +420.01 & +35.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
Consistent with the reject inference analysis of \textcite{crook2004reject}, the inclusion of parcelled rejected applications leads to a Gini shift of \textbf{\num{-0.0636}}. This shift is consistent with the presence of selection bias; through-the-door refitting produces a small, conservative change in the fitted parameters.
...
Reject Inference $\Delta$Gini & \textbf{\num{-0.0636}} & $0.00 - 0.10$ & \textcite{crook2004reject} & Below typical range & \\
...
\midrule
Champion (Scorecard) & 0.4009 & [0.3978, 0.4037] \\
Challenger (LightGBM) & 0.4067 & [0.4037, 0.4096] \\
\midrule
Difference (B $-$ A) & 0.0058 & [0.0049, 0.0065] \\
\bottomrule
...
\midrule
500 & 95.6\% & 20.16\% & \$283,658,169 & \$0 & 78.39\% \\
\textbf{530} & \textbf{64.4\%} & \textbf{14.22\%} & \textbf{\$98,545,014} & \textbf{\$0} & \textbf{58.80\%} \\
540 & 47.1\% & 11.48\% & \$52,961,853 & \$0 & 50.06\% \\
580 & 3.5\% & 3.39\% & \$1,123,705 & \$0 & 24.84\% \\
...
\midrule
Stage 1 (12-month ECL) & 487,338 & \$1.26bn & 0.00\% & 11.34\% & \$0.00bn \\
Stage 2 (lifetime ECL) & 291,659 & \$1.30bn & 0.00\% & 23.47\% & \$0.14bn \\
Stage 3 (credit-impaired) & 213,664 & \$1.17bn & 0.00\% & 22.72\% & \$1.04bn \\
\midrule
\textbf{Total IFRS 9 ECL} & & & & & \textbf{\$1.18bn} \\
...
LightGBM Gini (OOT)  & 0.4066 & $0.35 - 0.55$
  & \cite{lessmann2015benchmarking} & Within range & \\
...
IFRS~9 ECL Coverage  & 31.710\% & 40\% -- 45\%
